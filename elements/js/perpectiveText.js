...
export const handleTextSize = () => {
    const textSize = document.querySelector('.perspective-text__size');
    const textPerspective = document.querySelector('.perspective-text');

    const textSizeFunction = () => {
        const windowWidth = window.innerWidth;
        const textSizeRatio = windowWidth / 1000;
        const textSizeRatioValue = textSizeRatio > 1 ? 1 : (textSizeRatio < 1 && textSizeRatio > 0.7 ? textSizeRatio : 0.7);
...
export const handleAnimation = () => {
    ...
    const activateAnimation = () => {
        const perspectiveTextPosition = perspectiveText.offsetTop + 400;
